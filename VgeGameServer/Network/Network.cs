...
public static class Network
{
        ...
        try
        {
            ...
            if (state.message.Contains("$"))//如果包含结束符号
            {
                ...
                foreach (string splitedString in splitedStrings)
                {
                    ...
                    //派遣消息
                    NetMessageHandlerBase<NetMessageBase.PingPong>.Dispatch(state, splitedString);
                    //反序列化
                    //NetMessage_Old messages = JsonConvert.DeserializeObject<NetMessage_Old>(splitedString);
                    //派遣消息
    ...
    private static void CloseClientSocket(ClientState clientState)
    {
        ...
        //输出提示
        Console.WriteLine("客户端["+clientState.clientID+ "]失去了连接");
    }
    ...
    //public static void SendMessage(ClientState clientState, NetMessage_Old netMessage)
    //{
    //    //消息缓冲区
    ...
    /// <param name="clientState"></param>
    /// <param name="netMessage"></param>
    public static void SendMessage(ClientState clientState, JObject netMessage)
    {
        //消息缓冲区
        MemoryStream sendStream = new MemoryStream();
        //序列化消息（忽略空值）
    ...
    //public static void Broadcast(NetMessage_Old netMessage, ClientState negativeClient = null)
    //{
    ...
    /// <param name="netMessage">消息</param>
    /// <param name="negativeClient">指定哪个客户端不需要广播</param>
    public static void Broadcast(JObject netMessage, ClientState negativeClient = null)
    {
        List<ClientState> clientStates;
        if (negativeClient != null)//移除不需要广播的客户端
        {
    ...
    /// <param name="netMessage">消息</param>
    /// <param name="negativeClient">指定哪个客户端不需要广播</param>
    //public static void Broadcast(int roomID, NetMessage_Old netMessage, ClientState negativeClient = null)
    //{
    //    //获取指定房间的所有客户端
    ...
    /// <param name="netMessage">消息</param>
    /// <param name="negativeClient">指定哪个客户端不需要广播</param>
    public static void Broadcast(int roomID, JObject netMessage, ClientState negativeClient = null)
    {
        //获取指定房间的所有客户端
        List<ClientState> clientStates = clientSocketDict.Values.Where(t => PlayerManager.GetPlayer(t.clientID).roomID == roomID).ToList();
        //移除不需要广播的客户端
